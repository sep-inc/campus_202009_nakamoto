...
*/
	...
	public:
		...
		*/
		TronStage() :
			m_Stage{ Tron::ObjectType::TYPE_EMPTY },
			m_BlankStage{ Tron::ObjectType::TYPE_EMPTY }
		{
			// 上下の行を壁で埋める
			for (int i = 0; i < MAP_WIDTH; i++) {
				m_BlankStage[0][i] = Tron::ObjectType::TYPE_WALL;
				m_BlankStage[MAP_HEIGHT - 1][i] = Tron::ObjectType::TYPE_WALL;
			}

			// 左右の行を壁で埋める
			for (int i = 0; i < MAP_HEIGHT; i++) {
				m_BlankStage[i][0] = Tron::ObjectType::TYPE_WALL;
				m_BlankStage[i][MAP_WIDTH - 1] = Tron::ObjectType::TYPE_WALL;
			}
		}
