...
void TicTacToeGame::Update()
{
	...
	case TicTacToeGame::TicTacToeGameStep::STEP_INIT:
		...
		g_IsGameOver = false;
		g_WhosWon    = TicTacToe::ObjectType::TYPE_EMPTY;
		g_WhosTurn   = TicTacToe::ObjectType::TYPE_PlAYER;
		m_Player.Init();
		m_Enemy.Init();
		g_Grid.Init();
...
void TicTacToeGame::Announce()
{
	switch (g_WhosWon)
	{
	case TicTacToe::ObjectType::TYPE_PlAYER:
		printf("プレイヤーの勝利！！\n");
		break;

	case TicTacToe::ObjectType::TYPE_ENEMY:
		printf("エネミーの勝利！！\n");
		break;

	case TicTacToe::ObjectType::TYPE_EMPTY:
		printf("引き分け！！\n");
		break;
